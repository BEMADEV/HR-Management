...
", btnCopyToClipboard.ClientID );
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                var ptoTypeIdList = new PtoTypeService( new RockContext() ).Queryable().AsNoTracking().Where( pt => pt.IsActive ).Select( pt => pt.Id ).ToList().AsDelimited( "," );
                // Set URL in feed button
                var globalAttributes = GlobalAttributesCache.Get();
                btnCopyToClipboard.Attributes["data-clipboard-text"] = string.Format( "{0}/Webhooks/GetPtoCalendarFeed.ashx?PtoTypeIds={1}", globalAttributes.GetValue( "PublicApplicationRoot" ).EnsureTrailingForwardslash(), ptoTypeIdList );
