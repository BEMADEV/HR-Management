...
namespace RockWeb.Plugins.com_bemaservices.HrManagement
{
    ...
    [TextField( "Workflow Entry Page Route", "", true, "WorkflowEntry" )]
    [ContextAware]
    public partial class PtoRequestList : RockBlock, ICustomGridColumns
    {
        ...
        {
            gfPtoRequestFilter.DeleteFilterPreferences();
            BindFilter();
        }
        ...
        {
            gfPtoRequestFilter.SetFilterPreference( "Date Range", drpRequestDate.DelimitedValues );
            gfPtoRequestFilter.SetFilterPreference( "Status", ddlStatus.SelectedValue );
            gfPtoRequestFilter.SetFilterPreference( "Pto Type", ddlPtoType.SelectedValue );
            gfPtoRequestFilter.SetFilterPreference( "Person", ppPerson.PersonId.ToString() );
        ...
        protected void gPtoRequestList_Delete( object sender, RowEventArgs e )
        {
            if ( GetEditRights() )
            {
                ...
                {
                    var url = string.Format( "/{0}/{1}?PTORequest={2}&CancelRequest=Yes", GetAttributeValue( "WorkflowEntryPageRoute" ), workflowType.Id, ptoRequest.Guid.ToString() );
        ...
        protected void gPtoRequestList_Edit( object sender, RowEventArgs e )
        {
            if ( GetEditRights() )
            {
                ...
                if ( workflowType != null && ptoRequest != null )
                {
                    var url = string.Format( "/{0}/{1}?PTORequest={2}", GetAttributeValue( "WorkflowEntryPageRoute" ), workflowType.Id, ptoRequest.Guid.ToString() );
                    Response.Redirect( url, false );
                    Context.ApplicationInstance.CompleteRequest();
                }
            }
        ...
        protected void gPtoRequestList_Add( object sender, EventArgs e )
        {
            if ( GetEditRights() )
            {
                ...
                {
                    var url = string.Format( "/{0}/{1}", GetAttributeValue( "WorkflowEntryPageRoute" ), workflowType.Id );
                    if ( _person != null )
                    {
        ...
        private void BindFilter()
        {
            ...
            string statusFilter = gfPtoRequestFilter.GetFilterPreference( "Status" );
            ...
            ddlPtoType.SetValue( gfPtoRequestFilter.GetFilterPreference( "Pto Type" ) );
            ddlPtoType.DataBind();
            ddlPtoType.Items.Insert( 0, Rock.Constants.All.ListItem );

            drpRequestDate.DelimitedValues = gfPtoRequestFilter.GetFilterPreference( "Date Range" );

            var personId = gfPtoRequestFilter.GetFilterPreference( "Person" ).AsIntegerOrNull();
        ...
        protected void lRequestStatus_DataBound( object sender, RowEventArgs e )
        {
            ...
            PtoRequest ptoRequest = e.Row.DataItem as PtoRequest;

            var statusClass = string.Empty;
            switch ( ptoRequest.PtoRequestApprovalState )
            {
            ...
            lRequestStatus.Text = string.Format( "<span class='{0}'>{1}</span>", statusClass, ptoRequest.PtoRequestApprovalState.ConvertToString() );
        }
        ...
        private void BindGrid( bool isExporting = false )
        {
            ...
            gPtoRequestList.DataSource = ptoRequestQry.AsNoTracking().ToList();
        ...
        private IOrderedQueryable<PtoRequest> GetQuery( RockContext rockContext )
        {
            ...
            // filter by date
            string dateRangeValue = gfPtoRequestFilter.GetFilterPreference( "Date Range" );
            ...
            // filter by status
            var status = gfPtoRequestFilter.GetFilterPreference( "Status" ).ConvertToEnumOrNull<PtoRequestApprovalState>();
            if ( status.HasValue )
            {
            ...
            // filter by Pto Type
            var ptoTypeId = gfPtoRequestFilter.GetFilterPreference( "Pto Type" ).AsIntegerOrNull();
