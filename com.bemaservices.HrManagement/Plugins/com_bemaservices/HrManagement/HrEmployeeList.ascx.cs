                ...
                if ( person != null )
                {
                    ...
                    foreach ( var ptoType in ptoTypes.ToList() )
                    {
                        ...
                        if ( allocationPtoType != null )
                        {
                            ...
                            takenHours = allocationPtoType.Sum( pt => pt.PtoRequests.Where(pr=> pr.PtoRequestApprovalState != PtoRequestApprovalState.Cancelled && pr.PtoRequestApprovalState != PtoRequestApprovalState.Denied)
                                            .Sum( pr => pr.Hours ) );
