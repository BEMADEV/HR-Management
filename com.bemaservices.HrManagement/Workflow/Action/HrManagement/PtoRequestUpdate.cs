...
namespace com.bemaservices.HrManagement.Workflow.Action
{
    ...
    public class PtoRequestUpdate : ActionComponent
    {
        ...
        public override bool Execute( RockContext rockContext, WorkflowAction action, Object entity, out List<string> errorMessages )
        {
            ...
            var person = ptoAllocation.PersonAlias.Person;
            person.LoadAttributes();
            var tierGuid = person.GetAttributeValue( "PTOTier" ).AsGuidOrNull();

            if ( !tierGuid.HasValue )
            {
                errorMessages.Add( "The PTO Tier could not be found!" );
            }
            else
            {
                ptoTier = new PtoTierService(rockContext).Get(tierGuid.Value);
            }

            var weekendDays = new List<DayOfWeek>();
            if (  ptoTier == null || ptoTier.DaysOfWeek.IsNullOrWhiteSpace() )
            {
                weekendDays = "0,6".Split( ',' ).Select( a => ( DayOfWeek ) a.AsInteger() ).ToList();
            }
            else
            {
                weekendDays = ptoTier.DaysOfWeek.Split( ',' ).Select( a => ( DayOfWeek ) a.AsInteger() ).ToList();
            }
            ...
            if ( ptoAllocation != null && hours.HasValue && startDate.HasValue )
            {
                ...
                if ( ptoRequest != null )
                {
                    ...
                    if ( endDate.HasValue && endDate > startDate )
                    {
                        ...
                        {
                            if( !excludeWeekends || !weekendDays.Contains( requestDate.DayOfWeek) )
                            {
